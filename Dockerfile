...
RUN --mount=type=cache,id=openclaw-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    NODE_OPTIONS=--max-old-space-size=400 pnpm install --frozen-lockfile \
...
RUN NODE_OPTIONS=--max-old-space-size=400 pnpm_config_verify_deps_before_run=false pnpm build:docker
